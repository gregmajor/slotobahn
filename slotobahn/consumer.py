...
class Consumer(object):
    """This is a durable consumer that will handle unexpected events from RabbitMQ such as channel and connection
    closures. If RabbitMQ closes the connection, it will reopen it. You should look at the output, as there are limited
    reasons why the connection may be closed, which usually are tied to permission related issues or socket timeouts.
    ...
    def __init__(self, configuration, on_message):
        ...
        :param Function on_message: The function to call when a message is received.
        ...
        self._exchange = self._configuration.exchange
        self._exchange_type = self._configuration.exchange_type
    ...
    def add_on_connection_close_callback(self):
        """This method adds an on close callback that will be invoked by pika when RabbitMQ closes the connection to the
        publisher unexpectedly.
        """
    ...
    def on_bindok(self, unused_frame):
        """Invoked by pika when the Queue.Bind method has completed. At this point we will start consuming messages by
    ...
    def on_channel_open(self, channel):
        """This method is invoked by pika when the channel has been opened. The channel object is passed in so we can
        make use of it. Since the channel is now open, we'll declare the exchange to use.
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        """
        self._on_message(unused_channel, basic_deliver, properties, body)
    ...
    def on_queue_declareok(self, method_frame):
        ...
        will bind the queue and exchange together with the routing key by issuing the Queue.Bind RPC command. When this
    ...
    def setup_exchange(self, exchange_name):
        """Setup the exchange on RabbitMQ by invoking the Exchange.Declare RPC command. When it is complete, the
        on_exchange_declareok method will be invoked by pika.

        :param str|unicode exchange_name: The name of the exchange to declare

        exchange_declare(self,
                         callback=None,
                         exchange=None,
                         ...
                         type=None)
        """
        # This is commented out because we don't need to declare a new exchange.
    ...
    def setup_queue(self, queue_name):
        """Setup the queue on RabbitMQ by invoking the Queue.Declare RPC command. When it is complete, the
        on_queue_declareok method will be invoked by pika.
    ...
    def stop(self):
        """Cleanly shutdown the connection to RabbitMQ by stopping the consumer with RabbitMQ. When RabbitMQ confirms
        the cancellation, on_cancelok will be invoked by pika, which will then closing the channel and connection. The
        IOLoop is started again because this method is invoked when CTRL-C is pressed raising a KeyboardInterrupt
        exception. This exception stops the IOLoop which needs to be running for pika to communicate with RabbitMQ. All
        of the commands issued prior to starting the IOLoop will be buffered but not processed.
        """
